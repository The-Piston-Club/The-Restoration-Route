...
function saveLiveEditorLayout(){
  ...
  const payload={version:83,updatedAt:new Date().toISOString(),layout:DATA.layout,repairUiDesign:DATA.repairUiDesign};
...
function liveEditorUpdateJson(){const out=document.querySelector('#liveEditorPanel [data-json]'); if(!out)return; out.value=JSON.stringify({version:83,layout:DATA.layout,repairUiDesign:ACTIVE_REPAIR_DESIGN||DATA.repairUiDesign,layoutAdjustments},null,2);}
