...
let layoutAdjustments=loadLayoutAdjustments(), selectedLayoutEl=null, layoutEditorReady=false, repairFlowActive=false;
...
function setScales(){
  ...
  // Repair UI must use the exact same stage scale and anchor as the home UI.
  const repairScale=homeS;
  const repairLeft=homeLeft;
  const repairTop=homeTop;
  ...
  document.documentElement.style.setProperty("--repairScale",repairScale);
  ...
  document.documentElement.style.setProperty("--repairLeft",repairLeft+"px");
  document.documentElement.style.setProperty("--repairTop",repairTop+"px");
...
function requireLogin(){ if(layoutMode()||previewMode()||LOCAL_TEST_MODE)return true; if(!currentUser){openAuthPanel("register");return false;} if(!state.username||!state.termsAccepted){openAuthPanel("complete");return false;} return true; }
